...
//M_PI is not declared in the C standard, so declared it manually
#ifndef M_PI
...
typedef struct
{
    ...
    /* GPXData value. flexible array member */
...
typedef struct
{
    ...
    // assume that all waypoint children have no children of their own
...
typedef struct
{
    ...
    //multiple namespaces associated with it,  assume there is only one
    char namespace[256];
